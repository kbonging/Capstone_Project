...
export default function CampaignDetailModal({ isOpen, onClose, post, onChangeStatus }) {
  ...
  const modalContentClasses = `
    ...
    w-full max-w-[1400px] h-[80vh] max-h-[800px]
    ...
    <div className={modalBackdropClasses} onClick={onClose}>
      <div className={modalContentClasses} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-center border-b border-gray-200 p-5 bg-gray-50">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800">캠페인 상세</h2>
        ...
        {/* Main Grid */}
        <div className="flex flex-1 overflow-hidden p-6 gap-6 bg-gray-50">
          {/* 왼쪽: 썸네일 */}
          <div className="flex-1 flex justify-center items-center bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
            {post.thumbnailUrl ? (
              <img
          ...
          {/* 가운데 칸 (정보들) */}
          <div className="flex-1 flex flex-col gap-4 text-sm sm:text-base text-gray-900 overflow-auto pr-2">
            ...
            <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 flex flex-col gap-1">
              {(post.campaignTypeName === "배송형" || post.campaignTypeName === "구매형") && post.purchaseUrl && (
                <div>
                  ...
                  <a href={post.purchaseUrl} target="_blank" rel="noreferrer" className="text-blue-600 underline hover:text-blue-700">
                    {post.purchaseUrl}
                  </a>
                </div>
              )}
          ...
          {/* 오른쪽: 미션 */}
          <div className="flex-1 flex flex-col gap-2 p-4 bg-white rounded-xl shadow-inner overflow-auto border border-gray-100">
            <strong className="text-lg mb-2">미션</strong>
            <p className="whitespace-pre-line text-gray-700">{post.mission}</p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-gray-200 bg-gray-50">
          <button
            className="bg-green-500 text-white px-5 py-2 rounded-lg font-medium hover:bg-green-600 transition-colors"
          ...
          <button
            className="bg-gray-300 text-gray-800 px-5 py-2 rounded-lg font-medium hover:bg-gray-400 transition-colors"
            onClick={onClose}
          >
            닫기
          </button>
